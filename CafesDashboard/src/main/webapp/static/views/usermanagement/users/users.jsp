...
<c:choose>
	...
	>
		<div class="row">
			<div class="col-lg-12">
				...
				<h3>
					...
					>{{title}} </a> <span ng-show="showUser">/
						{{currentActiveOperation }} User</span>
				</h3>
			</div>
		</div>
		<div
		...
		>
			<div ng-show="showUser">
				<jsp:include page="/static/views/usermanagement/users/addUser.jsp" />
			</div>
			<div
	...
	<c:otherwise>
		<div
			id="clickBranchUserForm"
			emd-redirect-to="dashboard"
		><a id="clickBranchUserFormAnchor" href="#/branch-guests"></a></div>
	</c:otherwise>
</c:choose>
<script>
    ...
    $(document).ready(function () {
        $("#clickBranchUserForm").find('#clickBranchUserFormAnchor').trigger('click');
    });
</script>