...
angular.module('App').factory('UsersService',
		[ '$http', '$q', function($http, $q) {
			...
			var DELETED_USERS_URL = CONTEXT + '/get-deleted-users';
			...
			var GET_USER_BY_STATUS=CONTEXT+'/check-user-status';
			var GUEST_DELETE=CONTEXT+'/guest-delete/';
			...
			var factory = {
			    ...
			    fetchAllDeletedUsers:fetchAllDeletedUsers,
			...
			function fetchAllDeletedUsers() {
                var deferred = $q.defer();
                $http.get(DELETED_USERS_URL).then(function(response) {
                    deferred.resolve(response.data);
                }, function(errResponse) {
                    console.error('Error while fetching Users');
                    deferred.reject(errResponse);
			 ...
			 function getUserByStatus(email) {
                 var deferred = $q.defer();
                 $http.post(GET_USER_BY_STATUS, email).then(function(response) {
                     deferred.resolve(response.data);
                 }, function(errResponse) {
                     console.error('Error while Login');
                     deferred.reject(errResponse);
			 ...
			 function deleteUser(id) {
				 var deferred = $q.defer();
				 $http.delete(USER_DELETE+id).then(function (response) {
				 deferred.resolve(response.data);
				 },function(errResponse){
			 ...
			 function deleteGuest(id) {
                 var deferred = $q.defer();
                 $http.delete(GUEST_DELETE+id).then(function (response) {
                 deferred.resolve(response.data);
                 },function(errResponse){
                 console.error('Error while deleting role');
                 deferred.reject(errResponse);
